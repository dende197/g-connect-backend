...
def login_v2():
    ...
    try:
        ...
        if len(profiles) > 1 and selected_index is None:
            debug_log("👨‍👩‍👧‍👦 Più profili trovati, richiesta selezione")
            return jsonify({
                "success": True,
                "multi_profile": True,
                "profiles": profiles
        ...
        if selected_index is not None and 0 <= int(selected_index) < len(profiles):
            current_profile = profiles[int(selected_index)]
        ...
        import traceback
        debug_log("❌ ERRORE LOGIN V2", traceback.format_exc())
